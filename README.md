...
**隐私说明**：网页版只上传**答案解析**和**补充材料**的截图，**题目截图和文字不上传**——题目文字由 AI 提取后单独推送。你手机上看到的题目是文字版。云端数据用 PUSH_TOKEN 保护，网页版可再加 Cloudflare Access 登录墙（推荐）。
...
| 数据 | 是否上云 | 说明 |
| --- | --- | --- |
| 题目截图 | ❌ 不上传 | 网页端题目显示的是 AI 提取的文字 |
| 答案解析截图 | ✅ 上传 | 网页端答案直接看截图 |
| 补充材料截图 | ✅ 上传 | 同上 |
| 书籍/章节/题目元数据 | ✅ 上传 | 含考点、难度、掌握度、批注 |
| 本地图片 | ✅ 保留 | 本地永远双份（full+thumb） |
...
- **答案解析不上传文字**。桌面版答案只存截图（不提取文字），网页版答案也只看截图。
